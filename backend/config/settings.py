...
# Отдельный брокер/бэкэнд под тесты
...
MIDDLEWARE = [
    'corsheaders.middleware.CorsMiddleware',  # должно быть максимально высоко
...
# BOT_ADMINS = list(map(int, os.getenv('ADMINS', '').split(','))) if os.getenv('ADMINS') else []
...
# 2. Используем порт 587 (часто открыт, когда 465 закрыт)
EMAIL_PORT = 587

# 3. Для 587 порта настройки шифрования ТАКИЕ:
EMAIL_USE_SSL = False
EMAIL_USE_TLS = True
